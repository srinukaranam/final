...
# Add this function from the second code snippet
def migrate_db():
    with app.app_context():
        db = get_db()
        try:
...
if not os.path.exists(app.config['DATABASE']):
    ...
    # Add this line to run migrations on existing database
    migrate_db()

# Context processor to inject default variables to all templates
...
# Use the enhanced fetch function from the second code snippet
@retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, min=4, max=10))
...
# Enhanced company info function from the second code snippet
...
# Enhanced cache check function from the second code snippet
def get_cached_prediction(ticker, start_date, end_date):
    db = get_db()
    try:
        # Check if company_info column exists
...
# Enhanced visualization function from the second code snippet
    ...
    try:
        ...
        if cached:
            ...
            return render_template('app.html',
                                ...
                                from_cache=True,
                                current_date=start_date,  # Add these to fix template issues
        ...
        for date in pred_dates:
            ...
            predictions.append({
                ...
                'currency': currency_symbol  # Add currency for consistency
            })
